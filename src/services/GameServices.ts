...
export async function getCurrentGameSession(): Promise<GameSession> {
  const response = await http.get<GameSession>('/api/v1/game/sessions/current/');
  return response;
}
...
export async function resumeGame(): Promise<GameSession> {
  const response = await http.post<GameSession>('/api/v1/game/sessions/resume/');
  return response;
}
...
export async function updateGameTime(): Promise<{ game_session: GameSession; days_passed: number }> {
  const response = await http.post<{ game_session: GameSession; days_passed: number }>('/api/v1/game/sessions/update_time/');
  return response;
}
...
export async function addToBalance(amount: number, description: string): Promise<UserBalance> {
  const response = await http.post<UserBalance>('/api/v1/finance/balance/add/', {
    amount,
    description
  });
  return response;
...
export async function subtractFromBalance(amount: number, description: string): Promise<UserBalance> {
  const response = await http.post<UserBalance>('/api/v1/finance/balance/subtract/', {
    amount,
    description
  });
  return response;
...
export async function getGameDashboardData(): Promise<GameDashboardData> {
  const response = await http.get<GameDashboardData>('/api/v1/game/dashboard/data/');
  return response;
}
...
export function calculateGameProgress(currentDate: string, startDate: string, endDate: string): number {
  const current = new Date(currentDate);
  const start = new Date(startDate);
  const end = new Date(endDate);
  
  const totalDays = (end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24);
  const passedDays = (current.getTime() - start.getTime()) / (1000 * 60 * 60 * 24);
...
export async function getSuppliers(): Promise<Supplier[]> {
  const response = await http.get<Supplier[] | { results: Supplier[] }>('/api/v1/game/suppliers/');
  return Array.isArray(response) ? response : response.results || [];
}

// Serviços para produtos
export async function getProducts(): Promise<Product[]> {
  const response = await http.get<Product[] | { results: Product[] }>('/api/v1/game/products/');
  return Array.isArray(response) ? response : response.results || [];
}

export async function getProductsByCategory(categoryId: string): Promise<Product[]> {
  const response = await http.get<Product[] | { results: Product[] }>(`/api/v1/game/products/?category=${categoryId}`);
  return Array.isArray(response) ? response : response.results || [];
}

export async function getLowStockProducts(): Promise<Product[]> {
  const response = await http.get<Product[] | { results: Product[] }>('/api/v1/game/products/low_stock/');
  return Array.isArray(response) ? response : response.results || [];
}

export async function getOutOfStockProducts(): Promise<Product[]> {
  const response = await http.get<Product[] | { results: Product[] }>('/api/v1/game/products/out_of_stock/');
  return Array.isArray(response) ? response : response.results || [];
}

export async function purchaseProduct(productId: string, purchaseData: ProductPurchase): Promise<any> {
  const response = await http.post(`/api/v1/game/products/${productId}/purchase/`, purchaseData);
  return response;
}

// Serviços para histórico de estoque
export async function getProductStockHistory(): Promise<ProductStockHistory[]> {
  const response = await http.get<ProductStockHistory[] | { results: ProductStockHistory[] }>('/api/v1/game/stock-history/');
  return Array.isArray(response) ? response : response.results || [];
}

export async function getProductStockHistoryByProduct(productId: string): Promise<ProductStockHistory[]> {
  const response = await http.get<ProductStockHistory[] | { results: ProductStockHistory[] }>(`/api/v1/game/stock-history/?product=${productId}`);
  return Array.isArray(response) ? response : response.results || [];
}

// Serviços para vendas
export async function simulateSale(saleData: SaleSimulation): Promise<any> {
  const response = await http.post('/api/v1/game/sales/simulate_sale/', saleData);
  return response;
}

export async function getSalesSummary(): Promise<SalesSummary> {
  const response = await http.get<SalesSummary>('/api/v1/game/sales/sales_summary/');
  return response;
}
...
export function calculateTotalValue(unitPrice: number, quantity: number): number {
...
export function getStockStatus(currentStock: number, minStock: number): 'NORMAL' | 'LOW_STOCK' | 'OUT_OF_STOCK' {
...
export async function getRestockCost(): Promise<RestockCostResponse> {
  const response = await http.get<RestockCostResponse>('/api/v1/game/products/restock_cost/');
  return response;
}

export async function restockAllProducts(): Promise<RestockAllResponse> {
  const response = await http.post<RestockAllResponse>('/api/v1/game/products/restock_all/');
  return response;
}
